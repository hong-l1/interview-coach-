      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_440px]">
        <div className="panel rounded-lg p-8">
          <div className="mb-10 max-w-3xl">
            <div className="mb-3 inline-flex items-center gap-2 rounded-lg bg-[#57765a] px-3 py-2 text-sm font-semibold text-white">
              <ReadOutlined />
              校招综合面试
            </div>
            <h1 className="m-0 text-4xl font-black leading-tight text-[#182033]">
              从基础、项目和表达三条线，做一次校招面试预演。
            </h1>
            <p className="mt-4 text-base leading-7 text-[#667085]">
              适合应届生、实习和校招补强。前端会拉取你的简历列表，并把选中的简历传给综合面试 agent。
            </p>
          </div>

          <div className="grid gap-4 md:grid-cols-3">
            {[
              { icon: <BookOutlined />, title: '基础扎实度', desc: '数据结构、网络、操作系统、数据库等基础追问。' },
              { icon: <ProfileOutlined />, title: '简历可信度', desc: '围绕项目描述核验参与深度和技术边界。' },
              { icon: <ExperimentOutlined />, title: '成长潜力', desc: '关注学习能力、表达结构和问题拆解过程。' },
            ].map((item) => (
              <div key={item.title} className="surface rounded-lg p-5">
                <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-[#3157b7] text-lg text-white">
                  {item.icon}
                </div>
        ...
        <div className="panel rounded-lg bg-white p-6">
          ...
          {loadingResumes ? (
            ...
            <Form form={form} layout="vertical" initialValues={{ position: 'Java 后端开发', difficulty: '简单' }}>
              ...
              <Form.Item label="目标岗位" name="position" rules={[{ required: true, message: '请输入目标岗位' }]}>
                <Input placeholder="Java 后端开发" />
              </Form.Item>
              <Form.Item label="目标公司" name="company">
                <Input placeholder="可选，例如 美团" />
              </Form.Item>
